...
- Admin UI: `/admin` (`AdminPortal`); protected writes use `SCENE_SETTINGS_ADMIN_TOKEN` (operator-chosen secret in Netlify env, not in git); compact sidebar + large live previews; scene tab covers hero + configurator lighting, model quality, canvas backgrounds, and `configYaw`.
